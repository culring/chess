...
Shader::Shader(const GLchar* vertexPath, const GLchar* fragmentPath)
{
	...
	std::string vertex_code = read_shader_code(vertexPath);
	GLuint vertex_id = compile_shader(vertex_code.c_str(), GL_VERTEX_SHADER);

	std::string fragment_code = read_shader_code(fragmentPath);
	...
	glUniform1i(glGetUniformLocation(get_programID(), "skybox"), 0);
}
